...
#ifndef INCLUDED_PULSED_POWER_DAQ_RANGE_H
#define INCLUDED_PULSED_POWER_DAQ_RANGE_H
...
namespace gr {
  namespace pulsed_power_daq {
     ...
     */ 
    class PULSED_POWER_DAQ_API range_t
    {
      public:

        range_t(double value = 0.0) : d_start(value), d_stop(value), d_step(0.0) {}

        range_t(double start, double stop, double step = default_step) : d_start(start), d_stop(stop), d_step(step) {
          if (stop < start)
          {
    ...
    class PULSED_POWER_DAQ_API meta_range_t: public std::vector<range_t>
    {
      public:

        meta_range_t() { }
        meta_range_t(double start, double stop, double step = default_step) : std::vector<range_t> (1, range_t(start, stop, step)) { }

        template <typename InputIterator> meta_range_t(InputIterator first, InputIterator last, std::input_iterator_tag) : std::vector<range_t>(first, last) { /* NOP */ }
...
#endif /* INCLUDED_PULSED_POWER_DAQ_RANGE_H */