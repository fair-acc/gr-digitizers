...
#ifndef INCLUDED_PULSED_POWER_DAQ_TAGS_H
#define INCLUDED_PULSED_POWER_DAQ_TAGS_H
...
namespace gr {
  ...
  // ################################################################################################################
    ...
    struct PULSED_POWER_DAQ_API acq_info_t
    {
      int64_t timestamp;         // timestamp (UTC nanoseconds), used as fallback if no trigger is available
    ...
    struct PULSED_POWER_DAQ_API trigger_t
    {
     ...
     * \ingroup pulsed_power_daq
     */
    struct PULSED_POWER_DAQ_API wr_event_t
    {
      std::string event_id;
...
#endif /* INCLUDED_PULSED_POWER_DAQ_TAGS_H */