#ifndef _PS_5000A_DEFS_H_
#define _PS_5000A_DEFS_H_

#include <PicoStatus.h>
#include <string>

inline std::string ps5000a_status_to_string(PICO_STATUS status)
{
    switch (status) {
    case PICO_OK:
        return "PICO_OK";
...
inline std::string ps5000a_status_to_string_verbose(PICO_STATUS status)
{
    switch (status) {
    case PICO_OK:
...
inline std::string ps5000a_get_error_message(PICO_STATUS status)
{
    return std::string(ps5000a_status_to_string(status) + " - " +
                       ps5000a_status_to_string_verbose(status));
}
