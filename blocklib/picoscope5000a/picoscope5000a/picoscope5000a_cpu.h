...
#include <gnuradio/picoscope5000a/picoscope5000a.h>
...
class picoscope_5000a_impl : public digitizers::picoscope_impl
{
private:
    int16_t d_handle;   // picoscope handle
    int16_t d_overflow; // status returned from getValues
    const digitizer_resolution_t d_resolution;

public:
    picoscope_5000a_impl(const digitizers::digitizer_args& args,
                         digitizer_resolution_t resolution,
                         std::string serial_number,
                         logger_ptr logger);

    ~picoscope_5000a_impl();
...
private:
    ...
    uint32_t convert_frequency_to_ps5000a_timebase(double desired_freq,
                                                   double& actual_freq);
};

class picoscope5000a_cpu : public virtual picoscope5000a
{
public:
    explicit picoscope5000a_cpu(block_args args);
