                                             ...
                                             std::size_t first_digital = 0)
{
    ...
    for (auto i = first_digital; i <= 3u; ++i) {
        auto ns = blocks::null_sink::make({ .itemsize = sizeof(uint8_t) });
        fg->connect(ps, 8 + i, ns, 0);
    }
}

void qa_picoscope_5000a::open_close()
{
    auto ps = picoscope5000a::make({});
...
void qa_picoscope_5000a::rapid_block_basics()
{
...
void qa_picoscope_5000a::rapid_block_channels()
{
    ...
    connect_remaining_outputs_to_null_sinks(top, ps, 8 /*none*/, 2);
    top->run();
...
void qa_picoscope_5000a::rapid_block_continuous()
{
...
void qa_picoscope_5000a::rapid_block_downsampling_basics()
{
...
void qa_picoscope_5000a::run_rapid_block_downsampling(digitizer_downsampling_mode_t mode)
{
    auto top = gr::flowgraph::make("downsampling");
...
void qa_picoscope_5000a::rapid_block_downsampling()
{
...
void qa_picoscope_5000a::rapid_block_tags()
{
...
void qa_picoscope_5000a::rapid_block_trigger()
{
    auto top = gr::flowgraph::make("trigger");
...
void qa_picoscope_5000a::streaming_basics()
{
...
int main(int, char**)
{
    ...
    if (!var || std::string_view(var).find("5000a") == std::string_view::npos) {
        std::cout
            << "'5000a' not in PICOSCOPE_RUN_TESTS environment variable, do not run test"
            << std::endl;
        return 0;
    }
    ...
    runner.addTest(gr::picoscope5000a::qa_picoscope_5000a::suite());
