...
static PS6000_RANGE
convert_to_ps6000_range(float range) {
    if (range == 0.01)
        return PS6000_10MV;
    else if (range == 0.02)
        return PS6000_20MV;
...
convert_to_ps6000_threshold_direction(trigger_direction_t direction) {
    ...
    default:
        std::ostringstream message;
        message << "Exception in " << __FILE__ << ":" << __LINE__ << ": unsupported trigger direction:" << static_cast<int>(direction);
        throw std::runtime_error(message.str());
    }
};
...
convert_voltage_to_ps6000_raw_logic_value(double voltage, float channel_range) {
    if (fabs(voltage) > double(fabs(channel_range))) {
        std::ostringstream message;
        message << "Exception in " << __FILE__ << ":" << __LINE__ << ": Voltage '" << voltage << "' exceed maximum channel range (+/-" << channel_range << "V)";
#ifdef PORT_DISABLED // TODO can't access d_logger from free function
...
picoscope_6000_impl::picoscope_6000_impl(const digitizers::digitizer_args &args, std::string serial_number, logger_ptr logger)
...
std::string
picoscope_6000_impl::get_unit_info_topic(PICO_INFO info) const {
    char    line[40];
...
std::string
picoscope_6000_impl::get_driver_version() const {
...
std::string
picoscope_6000_impl::get_hardware_version() const {
    if (!d_initialized)
        return "NA";
...
picoscope_6000_impl::driver_configure() {
    ...
    if (d_trigger_settings.is_enabled()
        ...
        d_logger->info("Triggering enabled for picoscope: '{}' Trigger source: '{}' threshold: '{}' direction: '{}'", d_serial_number, d_trigger_settings.source, d_trigger_settings.threshold, static_cast<int>(d_trigger_settings.direction));
    } else {
        // disable triggers...
...
picoscope_6000_impl::driver_poll() {
    auto status = ps6000GetStreamingLatestValues(d_handle,
            (ps6000StreamingReady) digitizers::invoke_streaming_callback, &d_streaming_callback);
...
bool picoscope6000_cpu::start() {
    return d_impl.start();
}

bool picoscope6000_cpu::stop() {
    return d_impl.stop();
}

work_return_t picoscope6000_cpu::work(work_io &wio) {
    return d_impl.work(wio);
}

void picoscope6000_cpu::initialize() {
    d_impl.initialize();
}

void picoscope6000_cpu::close() {
    d_impl.close();
}
