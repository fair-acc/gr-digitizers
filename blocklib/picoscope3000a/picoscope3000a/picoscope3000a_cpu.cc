        ...
        if (status != PICO_OK) {
            ...
            if (status != PICO_OK) {
                ...
                message << "Exception in " << __FILE__ << ":" << __LINE__
                        ...
                        << " Error: " << ps3000a_get_error_message(status);
                throw std::runtime_error(message.str());
            }
...
std::error_code picoscope_3000a_impl::driver_initialize()
{
    ...
    if (status != PICO_OK) {
        ...
        GR_LOG_ERROR(d_logger,
                     "ps3000aMaximumValue: " + ps3000a_get_error_message(status));
        return make_pico_3000a_error_code(status);
    }
    ...
    if (status != PICO_OK) {
        ...
        GR_LOG_WARN(d_logger,
                    "ps3000aGetUnitInfo failed: " + ps3000a_get_error_message(status));
        GR_LOG_WARN(d_logger,
...
std::error_code picoscope_3000a_impl::driver_configure()
{
    ...
    if (d_acquisition_mode == acquisition_mode_t::RAPID_BLOCK) {
        ...
        if (status != PICO_OK) {
            GR_LOG_ERROR(d_logger,
                         "ps3000aMemorySegments: " + ps3000a_get_error_message(status));
            return make_pico_3000a_error_code(status);
        }
        ...
        if (status != PICO_OK) {
            GR_LOG_ERROR(d_logger,
                         "ps3000aSetNoOfCaptures: " + ps3000a_get_error_message(status));
            return make_pico_3000a_error_code(status);
        }
    }
    ...
    for (auto port = 0; port < d_ports; port++) {
        ...
        if (status != PICO_OK) {
            GR_LOG_ERROR(d_logger,
                         "ps3000aSetDigitalPort (port " + std::to_string(port) +
                             "): " + ps3000a_get_error_message(status));
            return make_pico_3000a_error_code(status);
        }
    }
    ...
    if (d_trigger_settings.is_analog() &&
        ...
        if (status != PICO_OK) {
            GR_LOG_ERROR(d_logger,
                         "ps3000aSetSimpleTrigger: " + ps3000a_get_error_message(status));
            return make_pico_3000a_error_code(status);
        }
    }
...
std::error_code picoscope_3000a_impl::driver_arm()
{
    if (d_acquisition_mode == acquisition_mode_t::RAPID_BLOCK) {
        ...
        if (status != PICO_OK) {
            GR_LOG_ERROR(d_logger,
                         "ps3000aRunBlock: " + ps3000a_get_error_message(status));
            return make_pico_3000a_error_code(status);
        }
    }
    else {
        ...
        if (status != PICO_OK) {
            GR_LOG_ERROR(d_logger,
                         "ps3000aRunStreaming: " + ps3000a_get_error_message(status));
            return make_pico_3000a_error_code(status);
        }
    }
...
std::error_code picoscope_3000a_impl::set_buffers(size_t samples, uint32_t block_number)
{
    ...
    for (auto aichan = 0; aichan < d_ai_channels; aichan++) {
        ...
        if (status != PICO_OK) {
            GR_LOG_ERROR(d_logger,
                         "ps3000aSetDataBuffer (chan " + std::to_string(aichan) +
                             "): " + ps3000a_get_error_message(status));
            return make_pico_3000a_error_code(status);
        }
    }
    ...
    for (auto port = 0; port < d_ports; port++) {
        ...
        if (status != PICO_OK) {
            GR_LOG_ERROR(d_logger,
                         "ps3000aSetDataBuffer (port " + std::to_string(port) +
                             "): " + ps3000a_get_error_message(status));
            return make_pico_3000a_error_code(status);
        }
    }
