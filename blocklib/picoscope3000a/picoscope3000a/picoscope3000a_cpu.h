...
#include <gnuradio/picoscope3000a/picoscope3000a.h>
...
class picoscope_3000a_impl : public digitizers::picoscope_impl {
private:
    int16_t d_handle;   // PicoScope device handle
    int16_t d_overflow; // status returned from getValues

public:
    picoscope_3000a_impl(const digitizers::digitizer_args &args, std::string serial_number, logger_ptr logger);
    ...
    std::error_code driver_get_rapid_block_data(size_t offset, size_t length, size_t waveform,
            work_io &wio, std::vector<uint32_t> &status) override;
...
class picoscope3000a_cpu : public virtual picoscope3000a {
public:
    explicit picoscope3000a_cpu(block_args args);
    ...
    std::string driver_version() const override {
        return d_impl.get_driver_version();
    }

    double actual_sample_rate() const override {
        return d_impl.get_samp_rate();
    }
    ...
    void set_aichan(std::string id, bool enabled, double range, digitizers::coupling_t coupling, double range_offset = 0) override {
    ...
    void set_aichan_trigger(std::string id, digitizers::trigger_direction_t trigger_direction, double threshold) {
        d_impl.set_aichan_trigger(id, trigger_direction, threshold);
    }

private:
    picoscope_3000a_impl d_impl;
};
