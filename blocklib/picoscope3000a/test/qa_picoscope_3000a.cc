...
namespace gr::picoscope3000a {
    ...
    void
    qa_picoscope_3000a::rapid_block_basics()
    {
      auto top = gr::flowgraph::make("basics");

      auto ps  = picoscope3000a::make({ .sample_rate = 10000.,
               .buffer_size                          = 1033, // TODO(PORT) calculate from pre+post
               .pre_samples                          = 33,
               .post_samples                         = 1000,
    ...
    void
    qa_picoscope_3000a::rapid_block_channels()
    {
      auto top = gr::flowgraph::make("channels");

      auto ps = picoscope3000a::make({
        .sample_rate = 10000.,
        .buffer_size = 1050, // TODO(PORT) calculate from pre+post
        .pre_samples = 50,
        .post_samples = 1000,
        .acquisition_mode = digitizer_acquisition_mode_t::RAPID_BLOCK,
        .rapid_block_nr_captures = 1,
    ...
    void
    qa_picoscope_3000a::rapid_block_continuous()
    {
      auto top = gr::flowgraph::make("continuous");

      auto ps = picoscope3000a::make({
        .sample_rate = 10000.0,
        .buffer_size = 1000, // TODO(PORT) calculate from pre+post
        .pre_samples = 0,
        .post_samples = 1000,
        .acquisition_mode = digitizer_acquisition_mode_t::RAPID_BLOCK,
        .rapid_block_nr_captures = 1,
    ...
    void
    qa_picoscope_3000a::rapid_block_downsampling_basics()
    {
      auto top = gr::flowgraph::make("downsampling");

      auto ps = picoscope3000a::make({
        .sample_rate = 10000.,
        .buffer_size = 1200, // TODO(PORT) calculate from pre+post
        .pre_samples = 200,
        .post_samples = 1000,
        .acquisition_mode = digitizer_acquisition_mode_t::RAPID_BLOCK,
        .rapid_block_nr_captures = 1,
    ...
    void
    qa_picoscope_3000a::run_rapid_block_downsampling(digitizer_downsampling_mode_t mode)
    {
        auto top = gr::flowgraph::make("channels");

        auto ps  = picoscope3000a::make({
                 .sample_rate             = 10000.,
                 .buffer_size             = 11000, // TODO(PORT) calculate from pre+post
                 .pre_samples             = 1000,
                 .post_samples            = 10000,
                 .acquisition_mode        = digitizer_acquisition_mode_t::RAPID_BLOCK,
                 .rapid_block_nr_captures = 1,
    ...
    void
    qa_picoscope_3000a::rapid_block_downsampling()
    {
    ...
    void
    qa_picoscope_3000a::rapid_block_tags()
    {
      auto top = gr::flowgraph::make("tags");

      auto samp_rate = 10000.0;

      auto ps        = picoscope3000a::make({ .sample_rate = samp_rate,
                     .buffer_size                          = 1200, // TODO(PORT) calculate from pre+post
                     .pre_samples                          = 200,
                     .post_samples                         = 1000,
                     .acquisition_mode                     = digitizer_acquisition_mode_t::RAPID_BLOCK,
      ...
      for (auto &tag: data_tags) {
          ...
          CPPUNIT_ASSERT(key == digitizers::acq_info_tag_name || key == digitizers::timebase_info_tag_name || key == digitizers::trigger_tag_name);
    ...
    void
    qa_picoscope_3000a::rapid_block_trigger()
    {
      auto top = gr::flowgraph::make("tags");
    ...
    void
    qa_picoscope_3000a::streaming_basics()
    {
      auto top = gr::flowgraph::make("streaming_basic");

      auto ps  = picoscope3000a::make({ .sample_rate = 1000.,
               .buffer_size                          = 100000,
...
int main(int, char **) {
    ...
    if (!var || std::string_view(var).find("3000a") == std::string_view::npos) {
        std::cout << "'3000a' not in PICOSCOPE_RUN_TESTS environment variable, do not run test" << std::endl;
        return 0;
    }
