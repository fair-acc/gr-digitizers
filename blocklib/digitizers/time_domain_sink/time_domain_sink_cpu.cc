...
time_domain_sink_cpu<T>::time_domain_sink_cpu(const typename time_domain_sink<T>::block_args &args)
    ...
    if (args.output_package_size == 0) {
        throw std::runtime_error(fmt::format("Exception in:{}:{} Channel: {} cannot set output_multiple to 0", __FILE__, __LINE__, args.signal_name));
    }
    ...
    try {
        ...
        throw std::runtime_error(fmt::format("Exception in:{}:{} Channel: {} Error: {}", __FILE__, __LINE__, args.signal_name, ex.what()));
    }
...
template<class T>
void time_domain_sink_cpu<T>::set_callback(std::function<void(std::vector<float>, std::vector<float>, std::vector<gr::tag_t>, void *)> cb, void *user_data) {
    d_cb_copy_data = cb;
...
work_return_t time_domain_sink_cpu<T>::work(work_io &wio) {
    ...
    const auto output_package_size = pmtf::get_as<std::size_t>(*this->param_output_package_size);
