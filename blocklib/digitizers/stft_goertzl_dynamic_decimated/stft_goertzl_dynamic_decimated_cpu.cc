...
stft_goertzl_dynamic_decimated_cpu::stft_goertzl_dynamic_decimated_cpu(const block_args &args)
    : INHERITED_CONSTRUCTORS {
    const double samp_rate_decimated = args.samp_rate / args.bounds_decimation;
    d_str2vec_sig                    = stream_to_vector_overlay::make({ args.window_size, args.samp_rate, args.delta_t });
    d_str2vec_min                    = stream_to_vector_overlay::make({ 1, samp_rate_decimated, args.delta_t });
    d_str2vec_max                    = stream_to_vector_overlay::make({ 1, samp_rate_decimated, args.delta_t });
