...
public:
    udp_receiver(boost::asio::io_service& io_service, udp::endpoint endpoint)
        : d_io_service(io_service),
          ...
          d_length(-1)
    {
        d_socket.async_receive(boost::asio::buffer(d_data, MAX_LENGTH),
                               boost::bind(&udp_receiver::handle_receive_from,
                                           this,
                                           boost::asio::placeholders::error,
                                           boost::asio::placeholders::bytes_transferred));
        d_thread.reset(
            new boost::thread(boost::bind(&boost::asio::io_service::run, &d_io_service)));
    }
