...
struct realignment_test_flowgraph_t {
    ...
    realignment_test_flowgraph_t(float user_delay,
                                 float triggerstamp_matching_tolerance,
                                 float buffer_time,
                                 const std::vector<tag_t>& tags,
                                 const std::vector<sim_wr_event_t>& wr_events,
                                 size_t data_size = 33333)
    {
        ...
        realign = gr::digitizers::time_realignment::make(
            { "id", user_delay, triggerstamp_matching_tolerance, buffer_time });
        for (auto event : wr_events)
            realign->add_timing_event(
                event.event_id, event.wr_trigger_stamp, event.wr_trigger_stamp_utc);
...
static void compare_static_fields(const acq_info_t& l, const acq_info_t& r)
{
...
void qa_time_realignment::default_case()
{
    ...
    realignment_test_flowgraph_t flowgraph(
        user_delay, triggerstamp_matching_tolerance, buffer_time, tags, wr_events);
...
void qa_time_realignment::no_timing()
{
    ...
    std::vector<gr::tag_t> tags = { make_acq_info_tag(tag, 100) };
    ...
    // CPPUNIT_ASSERT_DOUBLES_EQUAL(user_delay + tag.user_delay, acq_info.user_delay,
    // 1E-9); CPPUNIT_ASSERT_DOUBLES_EQUAL(user_delay + tag.actual_delay,
    // acq_info.actual_delay, 1E-9); CPPUNIT_ASSERT_DOUBLES_EQUAL( (double)tag.timestamp +
    // (user_delay * 1000000000.0), double)acq_info.timestamp, 1.0);
}
...
void qa_time_realignment::no_wr_events()
{
    ...
    std::vector<gr::tag_t> tags = { make_trigger_tag(tag1, 10000),
