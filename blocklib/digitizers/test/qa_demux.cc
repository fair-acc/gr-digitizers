...
struct extractor_test_flowgraph_t {
    ...
    std::vector<float> actual_values() { return value_sink->data(); }

    std::vector<float> actual_errors() { return error_sink->data(); }

    std::vector<gr::tag_t> tags() { return value_sink->tags(); }
};
                    ...
                    const std::vector<gr::tag_t>& tags = std::vector<gr::tag_t>{})
{
    ...
    flowgraph.value_src =
        gr::blocks::vector_source_f::make({ .data = values, .tags = tags });
    flowgraph.error_src = gr::blocks::vector_source_f::make({ .data = errors });
    flowgraph.extractor =
        gr::digitizers::demux::make({ post_trigger_window, pre_trigger_window });
...
void qa_demux::test_single_trigger()
{
    ...
    std::vector<gr::tag_t> tags = { make_trigger_tag(trigger_offset),
    ...
    //      {
    //          std::cout << "values expected - actual : " << values[sample_of_interest]
    //          << " - " << actual_values[i] << std::endl; sample_of_interest ++;
    //      }
...
void qa_demux::test_multi_trigger()
{
    ...
    std::vector<gr::tag_t> tags = {
        ...
        make_acq_info_tag(acq_info,
                          trigger3_offset +
                              199), // the trigger sample itself is the first sample
    ...
    //      {
    //          std::cout << "errors expected - actual : " << errors[sample_of_interest]
    //          << " - " << actual_errors[i] << std::endl; sample_of_interest ++;
    //      }
    ...
    CPPUNIT_ASSERT_EQUAL(uint64_t{ trigger_samples * 2 + pre_trigger_samples + 199 },
                         out_tags[5].offset());
}
...
void qa_demux::test_triggers_lost1()
{
    ...
    // Such a big number causes gnuradio to put a different number of items on
    // ninput_items[0] and ninput_items[1] !! Usually gnuradio blocks are called with up
    // to 20.000 samples. Not sure if we should write unittests for 1M samples at all
    ...
    flowgraph.run();
    //      std::cout << "flowgraph.tags().size(): " << flowgraph.tags().size() <<
    //      std::endl; for (auto tag : flowgraph.tags())
    //      {
    //          std::cout << "tag.offset: " << tag.offset << std::endl;
    //      }

    CPPUNIT_ASSERT_EQUAL((uint32_t)tags.size() * trigger_samples,
                         (uint32_t)flowgraph.actual_values().size());
    CPPUNIT_ASSERT_EQUAL((uint32_t)tags.size() * trigger_samples,
                         (uint32_t)flowgraph.actual_errors().size());
...
//
//      std::vector<gr::tag_t> tags;
//      for ( size_t offset = pre_trigger_samples; offset < data_size -
//      post_trigger_samples ; offset+=1 )
//          tags.push_back(make_trigger_tag(offset));
//
//      auto flowgraph = make_test_flowgraph(values, errors, pre_trigger_samples,
//      post_trigger_samples, tags);
//
//      flowgraph.run();
//
//      CPPUNIT_ASSERT_EQUAL((uint32_t)tags.size() * trigger_samples,
...
void qa_demux::test_hangup()
{
    ...
    size_t distance = 1;
    // Dont add gap at the ends for samples / presamples. This caused a hangup in the
    // previous implementation Tags which dont have enough samples to make up a full
    // window should just be ignored / processed in the next iteration
