...
public:
    ...
    public:
        ...
        void callback(std::span<float> values,
                      std::span<float> errors,
                      std::span<gr::tag_t> tags);

        void check_errors_zero();

        void check_values_equal(std::vector<float>& values);

        void check_errors_equal(std::vector<float>& errors);
