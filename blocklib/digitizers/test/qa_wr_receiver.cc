...
void qa_wr_receiver::test()
{
    ...
    // vector source is used to limit the data (flowgraph stops after data size samples is
    // generated)
    ...
    CPPUNIT_ASSERT_EQUAL(int64_t{ 123456789 },
                         event.wr_trigger_stamp_utc); // TODO(PORT) not in baseline
#ifdef PORT_DISABLED // TODO(PORT) this wasn't compiled in the baseline, check if it's
                     // good for anything
