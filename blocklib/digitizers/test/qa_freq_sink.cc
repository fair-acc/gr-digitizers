...
struct freq_test_flowgraph_t {
                          ...
                          std::size_t nvectors)
    {
        ...
        magnitude_src =
            gr::blocks::vector_source_f::make({ magnitude, false, vlen, tags });
        phase_src = gr::blocks::vector_source_f::make({ phase, false, vlen });
#ifdef PORT_DISABLED
        tag_debug = gr::blocks::tag_debug::make(
            { sizeof(float) * vlen, "tag_debug", acq_info_tag_name });
...
{
    (*static_cast<std::function<void(int64_t, std::string)>*>(ptr))(
        trigger_timestamp, std::move(signal_name));
}
...
void qa_freq_sink::test_metadata()
{
    ...
    auto sink = freq_sink::make({ name,
                                  SAMP_RATE_1KHZ,
                                  128,
                                  1,
                                  100,
                                  freq_sink_mode_t::FREQ_SINK_MODE_STREAMING });
...
void qa_freq_sink::test_sink_no_tags()
{
    ...
    ASSERT_VECTOR_EQUAL(
        fg.magnitude.begin(), fg.magnitude.end(), measurements.magnitude.begin());
...
void qa_freq_sink::test_sink_tags()
{
    ...
    freq_test_flowgraph_t fg(
        sink, std::vector<tag_t>{ make_acq_info_tag(info, 0) }, nbins, nmeasurements);
    fg.run();
    ...
    // CPPUNIT_ASSERT_EQUAL(info.timestamp, first.timestamp); FIXME: Test Fails since tag
    // restructurization
    ...
    for (std::size_t i = 1; i < nmeasurements; i++) {
        ...
        //   CPPUNIT_ASSERT_DOUBLES_EQUAL((double)expected_timestamp,
        //   (double)m.timestamp, 10.0);  FIXME: Test Fails since tag restructurization
...
void qa_freq_sink::test_sink_callback()
{
    ...
    auto callback = std::function<void(int64_t, std::string)>(
        [&counter, &local_event](int64_t trigger_timestamp, std::string signal_name) {
            local_event = { .trigger_timestamp = trigger_timestamp,
    ...
    ASSERT_VECTOR_EQUAL(fg.freq.begin(),
                        fg.freq.begin() + (nmeasurements / 2 * nbins),
                        measurements.frequency.begin());
}
