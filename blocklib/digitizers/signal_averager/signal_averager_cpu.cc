...
signal_averager_cpu::signal_averager_cpu(block_args args) : INHERITED_CONSTRUCTORS
{
    set_tag_propagation_policy(tag_propagation_policy_t::TPP_CUSTOM);
    set_relative_rate(
        1. / args.window_size); // TODO(PORT) This doesn't seem to have any effect?
}
...
work_return_t signal_averager_cpu::work(work_io& wio)
{
    ...
    for (std::size_t port = 0; port < wio.outputs().size(); port++) {
        ...
        for (i_out = 0; i_out < noutput_items; i_out++) {
            ...
            // required to merge acq_infotags due to this bug:
            // https://github.com/gnuradio/gnuradio/issues/2364 otherwise we will eat to
            // much memory
            // TODO: Fix bug in gnuradio
            // (https://gitlab.com/al.schwinn/gr-digitizers/issues/33)
            ...
            if (found_acq_info) {
                auto new_tag = make_acq_info_tag(
                    merged_acq_info, wio.outputs()[port].nitems_written() + i_out);
