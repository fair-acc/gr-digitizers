...
work_return_t post_mortem_sink_cpu::work(work_io& wio)
{
    ...
    memcpy(&d_buffer_values[d_write_index],
           wio.inputs()[0].items<float>(),
           ninput_items * sizeof(float));
    if (wio.outputs().size() > 1) {
        memcpy(wio.outputs()[0].items<float>(),
    ...
    if (reading_errors) {
        memcpy(&d_buffer_errors[d_write_index],
               wio.inputs()[1].items<float>(),
               ninput_items * sizeof(float));
    ...
    const auto tags =
        filter_tags(wio.inputs()[0].tags_in_window(0, ninput_items), acq_info_tag_name);
    if (!tags.empty()) {
...
{
    // TODO(PORT) do we really need this? why not make name/unit gettable? (and remove the
    // lock)
    std::scoped_lock lock{ d_mutex };
    return d_metadata;
}
...
post_mortem_data_t
post_mortem_sink_cpu::get_post_mortem_data(std::size_t nr_items_to_read)
{
    ...
    auto index =
        static_cast<int64_t>(d_write_index) - static_cast<int64_t>(nr_items_to_read);
    ...
    if (d_acq_info.timestamp < 0) {
        ...
        if (offset_first_sample >= d_acq_info_offset) {
            auto delta = d_acq_info.timebase * (offset_first_sample - d_acq_info_offset) *
                         1000000000.0;
            ret.info.timestamp = d_acq_info.timestamp + static_cast<uint64_t>(delta);
        }
        else {
            auto delta = d_acq_info.timebase * (d_acq_info_offset - offset_first_sample) *
                         1000000000.0;
            ret.info.timestamp = d_acq_info.timestamp - static_cast<uint64_t>(delta);
        }
    }
