...
template<class T>
time_realignment_cpu<T>::time_realignment_cpu(const typename time_realignment<T>::block_args &args)
...
template<class T>
work_return_t time_realignment_cpu<T>::work(work_io &wio) {
...
template<class T>
bool time_realignment_cpu<T>::add_timing_event(std::string event_id, int64_t wr_trigger_stamp, int64_t wr_trigger_stamp_utc) {
...
template<class T>
void time_realignment_cpu<T>::on_parameter_change(param_action_sptr action) {
    if (action->id() == time_realignment_cpu<T>::id_triggerstamp_matching_tolerance) {
    ...
    } else if (action->id() == time_realignment_cpu<T>::id_max_buffer_time) {
        d_max_buffer_time_ns = static_cast<int64_t>(pmtf::get_as<float>(*this->param_max_buffer_time) * BILLION);
    }
}

template<class T>
bool time_realignment_cpu<T>::fill_wr_stamp(trigger_t &trigger_tag_data) {
...
template<class T>
int64_t time_realignment_cpu<T>::get_user_delay_ns() const {
