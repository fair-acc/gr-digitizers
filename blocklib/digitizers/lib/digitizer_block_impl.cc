...
digitizer_block_impl::digitizer_block_impl(const digitizer_args &args, gr::logger_ptr logger)
...
void digitizer_block_impl::set_aichan(const std::string &id, bool enabled, double range, coupling_t coupling, double range_offset) {
...
void digitizer_block_impl::set_aichan_range(const std::string &id, double range, double range_offset) {
    auto idx                       = convert_to_aichan_idx(id);
    d_channel_settings[idx].range  = range;
    d_channel_settings[idx].offset = range_offset;
}

void digitizer_block_impl::set_aichan_trigger(const std::string &id, trigger_direction_t direction, double threshold) {
...
void digitizer_block_impl::set_diport(const std::string &id, bool enabled, double thresh_voltage) {
...
bool digitizer_block_impl::is_armed() {
    return d_armed;
}
    ...
    while (true) {
        ...
        if (state == poller_state_t::RUNNING) {
            ...
            if (estimated_samp_rate < (get_samp_rate() * WATCHDOG_SAMPLE_RATE_THRESHOLD)) {
                ...
                d_logger->error("Watchdog: estimated sample rate {}Hz, expected: {}Hz", estimated_samp_rate, get_samp_rate());
