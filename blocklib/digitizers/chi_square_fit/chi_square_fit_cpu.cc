      ...
      d_par_lower_limit(args.par_lim_dn)
{
    if (d_par_names.size() != args.n_params) {
        ...
        message << "Exception in " << __FILE__ << ":" << __LINE__
                << ": Parameter names do not match! must be of type:\"pName0\", "
                   "\"pName1\", ..., \"pName[n_params]\"";
...
{
    // check if input items is less than the minimal number of samples for a chi square
    // fit
