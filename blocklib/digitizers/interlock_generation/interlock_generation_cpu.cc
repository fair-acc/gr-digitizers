...
void interlock_generation_cpu::set_callback(std::function<void(int64_t, void*)> cb,
                                            void* user_data)
{
    d_callback = cb;
...
work_return_t interlock_generation_cpu::work(work_io& wio)
{
    ...
    for (std::size_t i = 0; i < noutput_items; i++) {
        ...
        const auto tags =
            filter_tags(wio.inputs()[0].tags_in_window(i, i + 1), acq_info_tag_name);
        ...
        if (d_interlock_issued && !interlock) {
            ...
            if (!tags.empty()) {
                ...
                if (d_acq_info.timestamp != -1) {
                    timestamp = d_acq_info.timestamp +
                                static_cast<int64_t>(((wio.inputs()[0].nitems_read() +
                                                       static_cast<uint64_t>(i)) -
                                                      tags.back().offset()) *
                                                     d_acq_info.timebase * 1000000000.0);
                }
            }
            if (d_callback) {
