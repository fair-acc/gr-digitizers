...
/*!
 * \brief Holding information about the timestamp (trigger or acquisition) when the
 * callback is called.
 *
 * Note, trigger_timestamp is the the same timestamp as provided to the time-realignment
 * block (via the add_timing_event method), else it contains the local timestamp of the
...
struct DIGITIZERS_API data_available_event_t {
    int64_t
        trigger_timestamp;   // trigger timestamp (without any realignment or user delay)
