...
function_cpu::function_cpu(block_args args)
    : INHERITED_CONSTRUCTORS, d_ref(args.ref), d_min(args.min), d_max(args.max)
{
    set_tag_propagation_policy(tag_propagation_policy_t::TPP_DONT);

    if (args.min.empty() || args.min.size() != args.ref.size() ||
        args.max.size() != args.ref.size() ||
        (args.timing.size() < (args.ref.size() - 1))) {
...
work_return_t function_cpu::work(work_io& wio)
{
    ...
    const auto min_noutput_items = std::min(
        { wio.outputs()[0].n_items, wio.outputs()[1].n_items, wio.outputs()[2].n_items });
    ...
    if (d_ref.size() == 1) {
        ...
        for (std::size_t i = 0; i < noutput_items; i++) {
            ...
            auto tags = filter_tags(
                wio.inputs()[0].tags_in_window(input_idx, input_idx + decimation),
                acq_info_tag_name);
            ...
            if (acq_info.timestamp == -1) {
                ...
                d_logger->error("function_ff_impl needs to be re-implemented, there is "
                                "no 'last_beam_in_timestamp' any more in acq_info");

                /*
                // timestamp of the first sample in working range
                auto timestamp = acq_info.timestamp + static_cast<int64_t>(((samp0_count +
                input_idx) - tags.at(0).offset) * acq_info.timebase * 1000000000.0);
                ...
                {
                  auto distance_from_beam_in = timestamp -
                acq_info.last_beam_in_timestamp;
