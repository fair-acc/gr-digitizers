 ...
 */
static float interpolateGaussian(const float *data, const int data_length, const int index) {
 ...
 * @return FWHM estimate [bins]
 */
static float computeInterpolatedFWHM(const float *data, const int data_length, const int index) {
    if ((index > 0) && (index < (data_length - 1))) {
        float maxHalf = 0.5 * data[index];
        int   lowerLimit;
        int   upperLimit;
