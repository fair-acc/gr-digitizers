...
struct Picoscope : gr::Block<Picoscope<T, TPSImpl, TTagMatcher>, gr::SupportedTypes<int16_t, float, gr::UncertainValue<float>, gr::DataSet<int16_t>, gr::DataSet<float>, gr::DataSet<gr::UncertainValue<float>>>> {
    ...
    A<gr::Tensor<std::pmr::string>, "IDs of enabled channels: `A`, `B`, `C` etc.">   channel_ids;
    ...
    A<gr::Tensor<std::pmr::string>, "Coupling modes of enabled channels">            channel_couplings;
    A<gr::Tensor<std::pmr::string>, "Signal names of enabled channels">              signal_names;
    A<gr::Tensor<std::pmr::string>, "Signal units of enabled channels">              signal_units;
    A<gr::Tensor<std::pmr::string>, "Signal quantity of enabled channels">           signal_quantities;
    ...
    void settingsChanged(const gr::property_map& oldSettings, const gr::property_map& newSettings) {
        ...
        for (const auto& [i, channelName] : std::views::zip(std::views::iota(0UZ), _picoscope->getChannelIds())) {
            ...
            if (auto j = findChannelSettingIndex(channelName)) {
                ...
                channelConfig.coupling = detail::convertToEnum<Coupling>(getChannelSetting(std::span(channel_couplings.value), *j, std::pmr::string{"DC"}));
            } else {
        ...
        for (std::size_t i = 0; i < channel_ids.value.size(); i++) {
            if (!configuredSuccessfully.contains(i)) {
                throw Error(std::format("Invalid channel name: {}", std::string_view(channel_ids.value[i])));
            }
        }
