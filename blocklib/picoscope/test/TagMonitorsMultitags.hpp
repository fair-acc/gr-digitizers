#ifndef GNURADIO_TAGMONITORS_NULTITAGS_HPP
#define GNURADIO_TAGMONITORS_NULTITAGS_HPP
...
struct TagSink : public Block<TagSink<T, UseProcessVariant>> {
    ...
    requires(UseProcessVariant == ProcessFunction::USE_PROCESS_BULK)
    {
        for (auto [idx, tagmap] : input.tags()) {
            if (verbose_console) {
                print_tag(gr::Tag{idx, tagmap}, std::format("{}::processBulk(...{})\t received tag at {:6}", this->name, input.size(), _nSamplesProduced + idx));
            }
            if (log_tags) {
...
#endif // GNURADIO_TAGMONITORS_NULTITAGS_HPP
