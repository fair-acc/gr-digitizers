...
test_streaming_basics() {
    ...
    auto            &ps          = flow_graph.make_node<Picoscope4000a<T>>({ { { "sample_rate", sample_rate },
...
const boost::ut::suite Picoscope4000aTests = [] {
    ...
    "open and close"_test = [] {
        gr::graph flow_graph;
        auto     &ps = flow_graph.make_node<Picoscope4000a<float>>();
    ...
    "rapid block basics"_test = [] {
        test_rapid_block_basic<float>(1);
        test_rapid_block_basic<int16_t>(1);
    };

    "rapid block multiple captures"_test = [] { test_rapid_block_basic<float>(3); };
    ...
    "rapid block 4 channels"_test        = [] {
        ...
        gr::graph             flow_graph;
        auto                 &ps    = flow_graph.make_node<Picoscope4000a<float>>({ { { "sample_rate", 10000. },
                                                                                      { "pre_samples", pre_samples },
                                                                                      { "post_samples", post_samples },
                                                                                      { "acquisition_mode", "RAPID_BLOCK" },
                                                                                      { "rapid_block_nr_captures", nr_captures },
