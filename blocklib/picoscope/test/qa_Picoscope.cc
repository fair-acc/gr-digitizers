...
// not used in the picooscope block with its processBulk function anyway.
...
void testRapidBlockBasic(std::size_t nCaptures, float sampleRate = 1234567.f, std::string triggerSource = "", bool testDigitalOutput = false) {
    ...
    if (!promptForTestCase(std::format("testRapidBlockBasic({}) - {}", nCaptures, gr::meta::type_name<T>()))) { return; }

    constexpr gr::Size_t preSamples   = 33;
    constexpr gr::Size_t postSamples  = 1000;
    const gr::Size_t     totalSamples = preSamples + postSamples;

    auto           digitalPortThreshold     = static_cast<int16_t>(32767 / 5);
...
void testStreamingBasics(float sampleRate = 83000.f, bool testDigitalOutput = false) {
    ...
    if (!promptForTestCase(std::format("testStreamingBasics - {} - {}", gr::meta::type_name<T>(), gr::meta::type_name<PicoscopeT>()))) { return; }
...
const boost::ut::suite PicoscopeTests = [] {
    ...
    "open and close"_test = []<PicoscopeImplementationLike PicoscopeT> {
        if (!promptForTestCase(std::format("open and close: {}", typeid(PicoscopeT).name()))) { return; }
        Graph flowGraph;
...
//    "rapid block digital output"_test = []<PicoscopeImplementationLike PicoscopeT> { testRapidBlockBasic<gr::DataSet<float>, PicoscopeT>(1, 4000, "", true); } | picoscopeTypes{};
    ...
    "rapid block 3 channels"_test = []<PicoscopeImplementationLike PicoscopeT> {
        if (!promptForTestCase(std::format("rapid block 3 channels: {}", typeid(PicoscopeT).name()))) { return; }
        constexpr gr::Size_t preSamples   = 33;
        constexpr gr::Size_t postSamples  = 1000;
    ...
    "rapid block continuous"_test = []<PicoscopeImplementationLike PicoscopeT> {
        if (!promptForTestCase(std::format("rapid block continous: {}", typeid(PicoscopeT).name()))) { return; }
        using namespace std::chrono_literals;
        ...
        expect(le(sinkA._nSamplesProduced, expectedDatasets));
        expect(ge(sinkA._nSamplesProduced, expectedDatasets / 2UZ)); // allow for
    ...
    "rapid block arm/disarm trigger"_test = []<PicoscopeImplementationLike PicoscopeT> {
        if (!promptForTestCase(std::format("rapid block arm/disarm trigger: {}", typeid(PicoscopeT).name()))) { return; }
        using namespace std::chrono_literals;
        using namespace gr::tag;
        ...
        expect(eq(sinkA._nSamplesProduced, 4UZ)) << fatal;
        for (auto & _sample : sinkA._samples) {
            expect(eq(_sample.signal_values.size(), totalSamples));
        }
    } | picoscopeTypes{};
    ...
    "rapid partial captures"_test = []<PicoscopeImplementationLike PicoscopeT> {
        ...
        // For this the test arms the scope for 4500 samples at 1kS/s, so for 4 seconds and requests 10 captures of 1005 samples => around 1 second each
        // The expected outcome is, that only the 4 updates occurring in the armed window will be published.
        if (!promptForTestCase(std::format("rapid partial captures: {}", std::string(typeid(PicoscopeT).name())))) { return; }
        using namespace std::chrono_literals;
        using namespace gr::tag;
