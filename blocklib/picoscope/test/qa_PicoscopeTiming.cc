...
#include "TagMonitorsMultitags.hpp" // #include <gnuradio-4.0/testing/TagMonitors.hpp>// TODO: upstream ability to check more than a single tag (also for tag propagation, not just monitoring directly after)
...
const boost::ut::suite PicoscopeTests = [] {
    ...
    "streamingWithTimingEdgeCases"_test = [&] {
        ...
        auto timingEventSamplesFromTags = sinkA._tags | std::views::filter([](auto& t) { return t.map.contains(gr::tag::CONTEXT.shortKey()); }) // only consider timing events
                                          | std::views::transform([](auto& t) { return t.index; })                                              // get tag index
                                          | std::ranges::to<std::vector>();
        // TODO: actually we expect 6, not 5
        expect(eq(timingEventSamplesFromTags.size(), 5UZ)) << [&]() { return std::format("expected to get exactly 6 timing tags, tags at samples: {}", timingEventSamplesFromTags); } << fatal;
        expect(std::ranges::equal(                                                                                                                                                                                                                                    // check that the tags are at the correct time points
            timingEventSamplesFromTags | std::views::transform([&](unsigned long v) -> double { return static_cast<double>(v - timingEventSamplesFromTags[0]) / static_cast<double>(kSampleRate) + 0.79; }), std::vector<double>{0.79, 0.8, 0.8, 1.2, 1.3 /*, 1.3*/}, // TODO: find out why/where one event gets swallowed and why sometimes the last event is off (+/-100us)
            [](auto a, auto b) { return std::abs(a - b) < 1e-9; }                                                                                                                                                                                                     // ignore differences smaller than 1ns
            ))
