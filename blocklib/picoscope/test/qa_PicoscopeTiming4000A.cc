...
const boost::ut::suite<"PicoscopeTimingTests"> PicoscopeTimingTests = [] {
    ...
    "streamingWithTimingSampleRate"_test = [&](const float samplingRate) {
        testStreamingWithTiming<TPSImpl>(samplingRate, 7s, "H");
        std::this_thread::sleep_for(100ms);
    } | std::array{1.0e3f, 10.0e3f, 100.0e3f};

    "triggeredTimingAcquisition"_test = [&](const float samplingRate) { testTriggeredAcquisitionWithTiming<TPSImpl>(samplingRate, 7s, "H"); } | std::array{1.0e4f, 1.0e5f};

    "triggeredTimingAcquisitionFast"_test = [&](const float samplingRate) { testTriggeredAcquisitionWithTiming<TPSImpl>(samplingRate, 7s, "H", 100us); } | std::array{20.0e6f}; // 80 MS is the maximum sample rate for this model, but since we use 3 channels, only use 20 MS
};
