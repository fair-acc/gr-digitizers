...
const boost::ut::suite<"PicoscopeTimingTests"> PicoscopeTimingTests = [] {
    ...
    "triggeredTimingAcquisition"_test = [&](const float samplingRate) { testTriggeredAcquisitionWithTiming<TPSImpl>(samplingRate, 7s, "D"); } | std::array{100000.0f};

    "triggeredTimingAcquisitionAux"_test = [&](const float samplingRate) { testTriggeredAcquisitionWithTiming<TPSImpl>(samplingRate, 7s, "AUX"); } | std::array{100000.0f};

    "triggeredTimingAcquisitionFast"_test = [&](const float samplingRate) { testTriggeredAcquisitionWithTiming<TPSImpl>(samplingRate, 7s, "D", 100us); } | std::array{200.0e6f};
};
