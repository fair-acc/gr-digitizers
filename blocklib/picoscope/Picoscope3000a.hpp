...
struct Picoscope3000a {
    ...
    [[nodiscard]] PICO_STATUS setDigitalBuffers(size_t nSamples, uint32_t segmentIndex, std::span<std::vector<int16_t>,N_DIGITAL_CHANNELS> digitalBuffers) const {
        for (std::size_t i = 0; i < N_DIGITAL_CHANNELS; i++) {
            ...
            const PICO_STATUS status = setDataBufferForSegment(static_cast<PS3000A_CHANNEL>(static_cast<int>(channelId)), digitalBuffers[i].data(), static_cast<int32_t>(nSamples), segmentIndex, ratioNone());
