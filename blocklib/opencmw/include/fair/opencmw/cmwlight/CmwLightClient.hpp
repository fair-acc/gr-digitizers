...
struct CmwLightError {
    using DataMap = std::map<std::string, std::variant<bool, int, long, std::string>>;
    int64_t     ContextAcqStamp{};
    int64_t     ContextCycleStamp{};
    std::string Message;
    int8_t      Type{};
    DataMap     ContextData{};
};
...
struct CmwLightRequestContext {
    using FilterMap = std::map<std::string, std::variant<bool, int, long, std::string>>;
    std::string x_8; // SELECTOR
    FilterMap   c;   // FILTERS
    FilterMap   x;   // DATA
    // accessors to make code more readable
    std::string& selector() { return x_8; };
    FilterMap&   filters() { return c; }
    FilterMap&   data() { return x; }
};
...
struct CmwLightDataContext {
    ...
    StringMap&   data() { return x; }
};

} // namespace opencmw::client::cmwlight
ENABLE_REFLECTION_FOR(opencmw::client::cmwlight::CmwLightError, ContextAcqStamp, ContextCycleStamp, Message, Type, ContextData)
 ...
 */
enum class MessageType : char { SERVER_CONNECT_ACK = 0x01, SERVER_REP = 0x02, SERVER_HB = 0x03, CLIENT_CONNECT = 0x20, CLIENT_REQ = 0x21, CLIENT_HB = 0x22 };

/**
 * Frame Types in the descriptor (Last frame of a message containing the type of each sub message)
 */
enum class FrameType : char { HEADER = 0, BODY = 1, BODY_DATA_CONTEXT = 2, BODY_REQUEST_CONTEXT = 3, BODY_EXCEPTION = 4 };

/**
 * request type used in request header REQ_TYPE_TAG
 */
enum class RequestType : char { GET = 0, SET = 1, CONNECT = 2, REPLY = 3, EXCEPTION = 4, SUBSCRIBE = 5, UNSUBSCRIBE = 6, NOTIFICATION_DATA = 7, NOTIFICATION_EXC = 8, SUBSCRIBE_EXCEPTION = 9, EVENT = 10, SESSION_CONFIRM = 11 };

/**
 * UpdateType
 */
enum class UpdateType : char { NORMAL = 0, FIRST_UPDATE = 1, IMMEDIATE_UPDATE = 2 };
...
inline std::string getIdentity() {
    ...
    static int  channelIdGenerator      = 1;
...
inline std::string createClientInfo() {
    ...
    std::string       username;
    username.reserve(32);
    if (const int usernameResult = getlogin_r(username.data(), username.capacity()); usernameResult == 0) {
        username.resize(strnlen(username.data(), username.size()));
    } else {
        username = "unknown";
    }
...
struct Connection {
    using SubscriptionMap = std::map<std::string, OpenSubscription, std::less<>>;
    using RequestMap      = std::map<std::string, PendingRequest, std::less<>>;
    ...
    SubscriptionMap                _subscriptions;                        // all subscriptions requested for (un)subscribe
    int64_t                        _subscriptionIdGenerator = 1;
    int64_t                        _requestIdGenerator      = 1;
    RequestMap                     _pendingRequests;

    Connection(const zmq::Context& context, const std::string_view authority, const int zmq_dealer_type) : _authority{authority}, _socket{context, zmq_dealer_type} { zmq::initializeSocket(_socket).assertSuccess(); }
};
...
static std::string descriptorToString(auto... descriptor) {
    ...
    ((result.push_back(std::to_underlying(descriptor))), ...);
    return result;
}
...
inline void sendConnectRequest(Connection& con) {
    ...
    detail::send(con._socket, ZMQ_SNDMORE, "error sending get frame"sv, std::to_underlying(MessageType::CLIENT_REQ));
    CmwLightHeader header;
    header.requestType() = std::to_underlying(RequestType::CONNECT);
...
class CMWLightClient : public CMWLightClientBase {
    ...
    constexpr static auto           MAX_BACKOFF        = 600s;

public:
    explicit CMWLightClient(const zmq::Context& context, std::vector<zmq_pollitem_t>& pollItems, DirectoryLightClient& directoryClient, const timeUnit timeout = 1s, std::string clientId = "") : _clientTimeout(timeout), _context(context), _directoryClient(directoryClient), _clientId(std::move(clientId)), _sourceName(std::format("CMWLightClient(clientId: {})", _clientId)), _pollItems(pollItems) {}

    void connect(detail::Connection& con) const {
        using enum detail::Connection::ConnectionState;
        using namespace std::string_view_literals;
        if (std::string id = detail::getIdentity(); !zmq::invoke(zmq_setsockopt, con._socket, ZMQ_IDENTITY, id.data(), id.size()).isValid()) { // hostname/process/id/channel -- the server verifies this
            throw std::runtime_error(std::format("failed set socket identity: {}", id));
        }
        if (auto endpoint = std::format("tcp://{}", con._authority); zmq::invoke(zmq_connect, con._socket, endpoint).isValid()) {
            _pollItems.push_back({.socket = con._socket.zmq_ptr, .fd = 0, .events = ZMQ_POLLIN, .revents = 0});
        } else {
            throw std::runtime_error(std::format("failed connect to server: {}", endpoint));
        }
        // send rda3 connect message
        detail::send(con._socket, ZMQ_SNDMORE, "error sending connect frame"sv, std::to_underlying(detail::MessageType::CLIENT_CONNECT));
    ...
    detail::Connection& findConnection(const URI<>& uri) {
        ...
        if (con == _connections.end()) {
            auto newCon        = detail::Connection(_context, uri.authority().value_or(""), ZMQ_DEALER);
    ...
    void get(const URI<>& uri, const std::string_view req_id) override {
        ...
        req.uri         = uri.str();
        con._pendingRequests.try_emplace(std::string(req_id), std::move(req));
    }
    ...
    void subscribe(const URI<>& uri, const std::string_view req_id) override {
        ...
        con._subscriptions.try_emplace(std::string(req_id), std::move(sub));
    }
    ...
    void unsubscribe(const URI<>& uri, const std::string_view req_id) override {
        ...
        CmwLightHeader header;
        header.requestType() = std::to_underlying(detail::RequestType::UNSUBSCRIBE);
    ...
    static bool handleServerReply(mdp::Message& output, detail::Connection& con, const auto currentTime) {
        ...
        if (con._connectionState == detail::Connection::ConnectionState::CONNECTING2) {
            if (header.requestType() == std::to_underlying(detail::RequestType::REPLY)) {
        ...
        case EXCEPTION: {
            ...
            IoBuffer         errorData(con._frames[2].data().data(), con._frames[2].data().size());
            DeserialiserInfo errorInfo = checkHeaderInfo<CmwLight>(errorData, DeserialiserInfo{}, ProtocolCheck::LENIENT);
            CmwLightError    error;
            auto             errorResult = opencmw::deserialise<CmwLight, ProtocolCheck::LENIENT>(errorData, error);
            output.error                 = std::format("Error in request: {} (AcqStamp:{}, CycleStamp:{}, Type:{})", error.Message, error.ContextAcqStamp, error.ContextCycleStamp, error.Type); /// std::string < UTF-8 strings containing  error code and/or stack-trace (e.g. "404 Not Found")
            return true;
        }
        case SUBSCRIBE: {
        ...
        case NOTIFICATION_DATA: {
            if (auto sub = std::ranges::find_if(con._subscriptions, [&header](auto& pair) { return pair.second.replyId == header.id(); }); sub == con._subscriptions.end()) {
                ...
                auto subscriptionForNotification = sub->second;
                output.arrivalTime               = std::chrono::system_clock::now();         /// timePoint   < UTC time when the message was sent/received by the client
        ...
        case NOTIFICATION_EXC: {
            ...
            if (auto subIt = con._subscriptions.find(replyId); subIt == con._subscriptions.end()) {
                ...
                CmwLightError    error;
                auto             errorResult = opencmw::deserialise<CmwLight, ProtocolCheck::LENIENT>(errorData, error);
                output.error                 = std::format("Error in subscription: {} (AcqStamp:{}, CycleStamp:{}, Type:{})", error.Message, error.ContextAcqStamp, error.ContextCycleStamp, error.Type); /// std::string < UTF-8 strings containing  error code and/or stack-trace (e.g. "404 Not Found")
                return true;
            }
        }
        ...
        case SERVER_CONNECT_ACK:
            if (con._connectionState == CONNECTING1) {
                if (con._frames.size() < 2 || con._frames[1].data().empty()) {
                    throw std::runtime_error("server connect message does not contain required version info");
                }
                if (con._frames[1].data() != "1.0.0") {
                    throw std::runtime_error(std::format("currently only version 1.0.0 is supported, server provided version: {}", con._frames[1].data()));
                }
            ...
            break;
        case SERVER_HB:
            if (con._connectionState != CONNECTED && con._connectionState != CONNECTING2) { // heart-beat message on an unconnected connection
                return false;
            }
            con._lastHeartbeatReceived = currentTime;
            break;
        case SERVER_REP: con._lastHeartbeatReceived = currentTime; return handleServerReply(output, con, currentTime);
        case CLIENT_CONNECT:
        case CLIENT_REQ:
        case CLIENT_HB:
        default: throw std::runtime_error("Unexpected client message type received from server");
        }
        return false;
    }
            ...
            case ConnectionState::CONNECTED:
                for (auto& req : con._pendingRequests | std::views::values) {
                    ...
                    if (req.state == RequestState::INITIALIZED) {
                        if (req.requestType == GET && !req.reqId.empty()) {
                            detail::send(con._socket, ZMQ_SNDMORE, "error sending get frame"sv, std::to_underlying(detail::MessageType::CLIENT_REQ));
                            CmwLightHeader msg;
                            msg.requestType() = std::to_underlying(GET);
                            ...
                            detail::send(con._socket, ZMQ_SNDMORE, "error sending get frame"sv, std::to_underlying(detail::MessageType::CLIENT_REQ));
                            CmwLightHeader msg;
                            msg.requestType() = std::to_underlying(SET);
                            char* reqIdEnd    = req.reqId.data() + req.reqId.size();
                            msg.id()          = std::strtol(req.reqId.data(), &reqIdEnd, 10);
                            msg.sessionId()   = detail::createClientId();
                            URI uri{req.uri};
                ...
                for (auto& sub : con._subscriptions | std::views::values) {
                    if (sub.state == INITIALIZED) {
                        detail::send(con._socket, ZMQ_SNDMORE, "error sending client req frame"sv, std::to_underlying(detail::MessageType::CLIENT_REQ));
                        URI            uri{sub.uri};
                        CmwLightHeader header;
                        header.id()          = sub.reqId;
                        header.device()      = uri.path()->substr(1, uri.path()->find('/', 1) - 1);
                        header.property()    = uri.path()->substr(uri.path()->find('/', 1) + 1);
                        header.requestType() = std::to_underlying(detail::RequestType::SUBSCRIBE);
                        header.options()     = std::make_unique<CmwLightHeaderOptions>();
                ...
                if (con._lastHeartBeatSent < now - HEARTBEAT_INTERVAL) {
                    detail::send(con._socket, 0, "error sending heart-beat"sv, std::to_underlying(detail::MessageType::CLIENT_HB));
                    con._lastHeartBeatSent = now;
                }
                if (con._lastHeartbeatReceived < now - HEARTBEAT_INTERVAL * 3) {
                    disconnect(con);
                    con._nextReconnectAttemptTimeStamp = now + con._backoff;
                    con._backoff                       = std::min(2 * con._backoff, std::chrono::duration_cast<std::chrono::milliseconds>(MAX_BACKOFF));
                    std::println("Missed 3 heartbeats -> connection seems to be broken");
                }
                break; // do nothing
            }
        }
...
public:
    explicit CmwLightClientCtx(const zmq::Context& zeromq_context, std::string nameserver, const timeUnit timeout = 100ms, std::string clientId = "") : _zctx{zeromq_context}, _nameserver{DirectoryLightClient{std::move(nameserver)}}, _control_socket_send(zeromq_context, ZMQ_PAIR), _control_socket_recv(zeromq_context, ZMQ_PAIR), _timeout(timeout), _clientId(std::move(clientId)) {
        zmq::invoke(zmq_bind, _control_socket_send, "inproc://cmwlightclientControlSocket").onFailure<std::runtime_error>("could not bind control socket");
    ...
    std::unique_ptr<CMWLightClientBase>& getClient(const URI<>& uri) {
        ...
        auto [it, ins] = _clients.emplace(baseUri, createClient(_nameserver));
        if (!ins) {
    ...
    std::unique_ptr<CMWLightClientBase> createClient(DirectoryLightClient& nameserver) { return std::make_unique<CMWLightClient>(_zctx, _pollitems, nameserver, _timeout, _clientId); }
    ...
    void request(Command cmd) override {
        ...
        if (cmd.command == mdp::Command::Get || cmd.command == mdp::Command::Set) {
            req_id = _request_id++;
            _requests.try_emplace(req_id, Request{.uri = cmd.topic, .callback = std::move(cmd.callback), .timestamp_received = cmd.arrivalTime});
        } else if (cmd.command == mdp::Command::Subscribe) {
            req_id                      = _request_id++;
            std::string clientRequestId = std::format("{}", req_id);
            _subscriptions.try_emplace(clientRequestId, Subscription{.uri = cmd.topic, .callback = std::move(cmd.callback), .timestamp_received = cmd.arrivalTime, .reqId = req_id});
    ...
    DirectoryLightClient& nameserverClient() { return _nameserver; };

private:
    void sendCmd(const URI<>& uri, mdp::Command commandType, const std::size_t req_id, IoBuffer data = {}) const {
    ...
    void handleRequests() {
        ...
        while (cmd.receive(_control_socket_recv, ZMQ_DONTWAIT).isValid()) {
            ...
            } else if (cmd.data()[0] == std::to_underlying(mdp::Command::Get)) {
                client->get(uri, reqId.data());
            } else if (cmd.data()[0] == std::to_underlying(mdp::Command::Set)) {
            ...
            } else if (cmd.data()[0] == std::to_underlying(mdp::Command::Subscribe)) {
                client->subscribe(uri, reqId.data());
            } else if (cmd.data()[0] == std::to_underlying(mdp::Command::Unsubscribe)) {
    ...
    void poll(const std::stop_token& stoken) {
        using enum mdp::Command;
        thread::setThreadName("CmwLightPollerThread");
        auto nextHousekeeping = std::chrono::system_clock::now();
        zmq::invoke(zmq_connect, _control_socket_recv, "inproc://cmwlightclientControlSocket").assertSuccess();
        while (!stoken.stop_requested() && zmq::invoke(zmq_poll, _pollitems.data(), static_cast<int>(_pollitems.size()), 200)) {
            ...
            for (const auto& client : _clients | std::views::values) {
                ...
                while (client->receive(receivedEvent)) {
                    ...
                    if (receivedEvent.command == Invalid) { // Protocol internal messages, which require further receives but no publishing
                        continue;
                    }
                    if (std::string clientRequestId{reinterpret_cast<char*>(receivedEvent.clientRequestID.data()), receivedEvent.clientRequestID.size()}; receivedEvent.command == Notify && _subscriptions.contains(clientRequestId)) {
                        _subscriptions.at(clientRequestId).callback(receivedEvent); // callback
                    }
                    if (receivedEvent.command == Final && _requests.contains(receivedEvent.id)) {
