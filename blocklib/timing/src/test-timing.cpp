...
std::pair<uint64_t, uint64_t> TimingGroupFilterDropdown() {
    ...
    ImGui::RadioButton("All Timing Groups", &radioButtonState, 0);
    ImGui::SameLine();
    ImGui::RadioButton("SIS18", &radioButtonState, 1);
    ImGui::SameLine();
    ImGui::RadioButton("SIS100", &radioButtonState, 2);
    ImGui::SameLine();
    ImGui::RadioButton("ESR", &radioButtonState, 3);
    ImGui::SameLine();
    ImGui::RadioButton("CRYRING", &radioButtonState, 4);
    ImGui::SameLine();
    ImGui::RadioButton("Other:", &radioButtonState, 5);
    ImGui::SameLine();
    ...
    case 0: return {0, 0};
    case 1: return getTimingMask(300);
    case 2: return getTimingMask(310);
    case 3: return getTimingMask(340);
    case 4: return getTimingMask(210);
    default: return {result[static_cast<std::size_t>(current)], mask};
    }
}
...
bool drawScheduledEventTableRow(Timing::Event& ev, Timing& timing, uint64_t default_offset) {
    ...
    if (ImGui::Button("inject")) {
        timing.injectEvent(ev, timing.currentTimeTAI() + default_offset);
    }
    ...
    if (ImGui::CollapsingHeader("SchedULe to inject", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        static ImGuiTableFlags flags = ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY | ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersOuter | ImGuiTableFlags_BordersV | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable;
        {
            ...
            if (auto _1 = ImScoped::Table("event schedule", static_cast<int>(21 + timing.outputs.size()), flags, outer_size, 0.f)) {
                ...
                timing.events.erase(std::remove_if(timing.events.begin(), timing.events.end(), [&timing, &default_offset_ = default_offset](auto& ev) { return drawScheduledEventTableRow(ev, timing, default_offset_); }), timing.events.end());
            }
        }
    }
...
void conditionsTable(const Timing& timing, const float TEXT_BASE_HEIGHT) {
    ...
    static ImGuiTableFlags flags_conds = ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY | ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersOuter | ImGuiTableFlags_BordersV | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable;
