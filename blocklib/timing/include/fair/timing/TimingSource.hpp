...
```
    ...
    A<gr::Tensor<std::pmr::string>, "event actions", Doc<"Configure which timing events should trigger IO port changes and/or get forwarded as timing tags. Syntax description and examples in the block documentation.">, Visible> event_actions;
    ...
    void settingsChanged(const property_map& oldSettings, const property_map& newSettings) {
        ...
        if (newSettings.contains("event_actions") && event_actions != oldSettings.at("event_actions").value_or(gr::Tensor<std::pmr::string>{})) {
            if (_timing.receiver) {
                updateEventTriggers();
            }
        } // end io triggers
