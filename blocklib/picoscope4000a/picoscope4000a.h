...
enum class digitizer_acquisition_mode_t { STREAMING, RAPID_BLOCK };
...
struct Settings {
    ...
    digitizer_acquisition_mode_t acquisition_mode =
        digitizer_acquisition_mode_t::STREAMING;
...
struct Picoscope4000a : public fair::graph::node<Picoscope4000a> {
    ...
    explicit Picoscope4000a(Settings settings_ = {})
    ...
    available_samples(const Picoscope4000a&) const noexcept
    {
                                                   ...
                                                   std::span<float> errors) noexcept
    {
        ...
        assert(state.data_available >= values.size());
        const auto data = state.channels[0].data_reader.get(values.size());
        std::ignore = std::copy(data.begin(), data.end(), values.begin());
        const auto error_estimate = state.channels[0].settings.range *
                                    static_cast<double>(DRIVER_VERTICAL_PRECISION);
        std::fill(errors.begin(), errors.end(), error_estimate);
        std::ignore = state.channels[0].data_reader.consume(data.size());
        state.data_available -= data.size();
    ...
    void start() noexcept
    {
        ...
        try {
            initialize();
            configure();
            if (ps_settings.acquisition_mode == digitizer_acquisition_mode_t::STREAMING) {
    ...
    void initialize()
    {
        ...
        const auto ec = driver_initialize();
        if (ec) {
    ...
    // implementation for PS subtypes (TODO move to separate class used as template
    // parameter)
    ...
    std::error_code driver_set_buffers(size_t samples, uint32_t block_number);
};
