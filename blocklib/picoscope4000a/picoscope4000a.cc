...
convert_frequency_to_ps4000a_timebase(int16_t handle, double desired_freq, double &actual_freq) {
    ...
    for (std::size_t i = 0; i < 2; i++) {
        ...
        if (status != PICO_OK) {
            ...
            if (status != PICO_OK) {
                throw std::runtime_error(fmt::format("Local time {}. Error: {}", timebase_estimate, ps4000a_get_error_message(status)));
            }
...
void
rapid_block_callback_redirector(int16_t handle, PICO_STATUS status, void *vobj) {
    std::ignore = handle;
...
Picoscope4000a::driver_channel_id_to_index(std::string_view id) {
...
std::string
Picoscope4000a::driver_driver_version() const {
...
std::string
Picoscope4000a::driver_hardware_version() const {
...
std::error_code
Picoscope4000a::driver_initialize() {
    PICO_STATUS status;
    ...
    if (status != PICO_OK) {
        fmt::println(std::cerr, "open unit failed: {} ", ps4000a_get_error_message(status));
    ...
    if (status != PICO_OK) {
        ...
        fmt::println(std::cerr, "ps4000aMaximumValue: {}", ps4000a_get_error_message(status));
        return make_pico_4000a_error_code(status);
    }
...
std::error_code
Picoscope4000a::set_buffers(std::size_t samples, uint32_t block_number) {
    for (auto &channel : state.channels) {
        ...
        if (status != PICO_OK) {
            fmt::println(std::cerr, "ps4000aSetDataBuffer (chan {}): {}", static_cast<std::size_t>(*channel_index), ps4000a_get_error_message(status));
            return make_pico_4000a_error_code(status);
        }
    }
...
Picoscope4000a::driver_close() {
    ...
    if (status != PICO_OK) {
        fmt::println(std::cerr, "ps4000aCloseUnit: {}", ps4000a_get_error_message(status));
    }
    return make_pico_4000a_error_code(status);
}

std::error_code
Picoscope4000a::driver_configure() {
    int32_t max_samples;
    auto    status = ps4000aMemorySegments(state.handle, static_cast<uint32_t>(ps_settings.rapid_block_nr_captures), &max_samples);
    if (status != PICO_OK) {
        fmt::println(std::cerr, "ps4000aMemorySegments: {}", ps4000a_get_error_message(status));
        return make_pico_4000a_error_code(status);
    }
    ...
    if (ps_settings.acquisition_mode == acquisition_mode_t::RAPID_BLOCK) {
        ...
        if (status != PICO_OK) {
            fmt::println(std::cerr, "ps4000aSetNoOfCaptures: {}", ps4000a_get_error_message(status));
            return make_pico_4000a_error_code(status);
        }
    }
    ...
    for (const auto &channel : state.channels) {
        ...
        if (status != PICO_OK) {
            fmt::println(std::cerr, "ps4000aSetChannel (chan '{}'): {}", channel.id, ps4000a_get_error_message(status));
            return make_pico_4000a_error_code(status);
        }
    }
    ...
    if (trigger.value.is_analog() && ps_settings.acquisition_mode == acquisition_mode_t::RAPID_BLOCK) {
        ...
        status = ps4000aSetSimpleTrigger(state.handle,
                                         true, // enable
                                         *channel, convert_voltage_to_ps4000a_raw_logic_value(ps_settings.trigger.threshold), convert_to_ps4000a_threshold_direction(ps_settings.trigger.direction),
                                         0,   // delay
                                         -1); // auto trigger
        if (status != PICO_OK) {
            fmt::println(std::cerr, "ps4000aSetSimpleTrigger: {}", ps4000a_get_error_message(status));
            return make_pico_4000a_error_code(status);
        }
    } else {
        // disable triggers
        for (int i = 0; i < PS4000A_MAX_CHANNELS; i++) {
            ...
            if (status != PICO_OK) {
                fmt::println(std::cerr, "ps4000aSetTriggerChannelConditionsV2: {}", ps4000a_get_error_message(status));
                return make_pico_4000a_error_code(status);
            }
        }
    }
...
std::error_code
Picoscope4000a::driver_arm() {
    if (ps_settings.acquisition_mode == acquisition_mode_t::RAPID_BLOCK) {
        uint32_t timebase = convert_frequency_to_ps4000a_timebase(state.handle, ps_settings.sample_rate, state.actual_sample_rate);
        ...
        auto     status   = ps4000aRunBlock(state.handle, static_cast<int32_t>(ps_settings.pre_samples), static_cast<int32_t>(ps_settings.post_samples),
                                            ...
                                            0,        // segment index
                                            static_cast<ps4000aBlockReady>(rapid_block_callback_redirector), this);
        if (status != PICO_OK) {
        ...
        if (status != PICO_OK) {
            fmt::println(std::cerr, "ps4000aRunStreaming: {}", ps4000a_get_error_message(status));
            return make_pico_4000a_error_code(status);
        }
    }
...
std::error_code
Picoscope4000a::driver_disarm() noexcept {
