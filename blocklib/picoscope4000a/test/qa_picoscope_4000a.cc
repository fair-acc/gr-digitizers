...
#ifdef PORT_DISABLED // TODO(PORT) these are currently not settable after creation, is this needed?
...
void qa_picoscope_4000a::rapid_block_continuous() {
    ...
    auto ps  = picoscope4000a::make({ .sample_rate = 10000,
             .buffer_size                          = 1000,
             .post_samples                         = 1000,
             .acquisition_mode                     = digitizer_acquisition_mode_t::RAPID_BLOCK, // TODO(PORT) buffer_size was set explicitly before when setting pre/post samples
             .rapid_block_nr_captures              = 1 });
...
int main(int, char **) {
    ...
    if (!var || std::string_view(var).find("4000a") == std::string_view::npos) {
        std::cout << "'4000a' not in PICOSCOPE_RUN_TESTS environment variable, do not run test" << std::endl;
        return 0;
    }
