...
#include <pulsed_power_daq/picoscope_base.h>
...
namespace gr {
  namespace pulsed_power_daq {
    ...
    picoscope_base::picoscope_base(std::string serial_number, int max_ai_channels, int max_di_ports,
            bool auto_arm, int16_t max_raw_analog_value, float vertical_precision)
      : digitizer_source(max_ai_channels, max_di_ports, auto_arm),
        ...
        d_ranges(),
        d_streaming_callback(boost::bind(&picoscope_base::streaming_callback, this, _1, _2, _3)),
    ...
    picoscope_base::get_aichan_ids()
    {
      return d_channel_ids;
    }

    meta_range_t
    picoscope_base::get_aichan_ranges()
    {
      return d_ranges;
    }

    void
    picoscope_base::streaming_callback(int32_t nr_samples, uint32_t start_index, int16_t overflow)
    {
      // trigger timestamp
