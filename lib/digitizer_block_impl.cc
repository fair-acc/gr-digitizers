...
// Relevant for debugging. If gnuradio calls this block not often enough, we
// will get "WARN: XX digitizer data buffers lost" The rate in which this block
// is called is given by the number of free slots on its output buffer. So we
// choose a big value via set_min_output_buffer
uint64_t last_call_utc = 0;

static const int AVERAGE_HISTORY_LENGTH = 100000;

digitizer_block_impl::digitizer_block_impl(int ai_channels, int di_ports,
                                           bool auto_arm)
    : d_samp_rate(10000), d_actual_samp_rate(d_samp_rate),
      d_time_per_sample_ns(1000000000. / d_samp_rate), d_pre_samples(1000),
      d_post_samples(9000), d_nr_captures(1), d_buffer_size(8192),
      d_nr_buffers(100), d_driver_buffer_size(100000),
      d_acquisition_mode(acquisition_mode_t::STREAMING), d_poll_rate(0.001),
      d_downsampling_mode(downsampling_mode_t::DOWNSAMPLING_MODE_NONE),
      d_downsampling_factor(1), d_ai_channels(ai_channels), d_ports(di_ports),
      d_channel_settings(), d_port_settings(), d_trigger_settings(),
      d_status(ai_channels), d_app_buffer(),
      d_was_last_callback_timestamp_taken(false),
      d_estimated_sample_rate(AVERAGE_HISTORY_LENGTH), d_initialized(false),
      d_closed(false), d_armed(false), d_auto_arm(auto_arm),
      d_trigger_once(false), d_was_triggered_once(false),
      d_timebase_published(false), ai_buffers(ai_channels),
      ai_error_buffers(ai_channels), port_buffers(di_ports), d_data_rdy(false),
      d_trigger_state(0), d_read_idx(0), d_buffer_samples(0), d_errors(128),
...
void digitizer_block_impl::add_error_code(std::error_code ec) {
  d_errors.push(ec);
}

std::vector<int>
digitizer_block_impl::find_analog_triggers(float const *const samples,
                                           int nsamples) {
...
std::vector<int>
digitizer_block_impl::find_digital_triggers(uint8_t const *const samples,
                                            int nsamples, uint8_t mask) {
...
double digitizer_block_impl::get_samp_rate() { return d_actual_samp_rate; }
...
void digitizer_block_impl::set_downsampling(downsampling_mode_t mode,
  ...
  if (mode == downsampling_mode_t::DOWNSAMPLING_MODE_NONE) {
    ...
    std::ostringstream message;
    message << "Exception in " << __FILE__ << ":" << __LINE__
            << ": downsampling factor should be at least 2: "
            << downsample_factor;
    throw std::invalid_argument(message.str());
  }
...
void digitizer_block_impl::set_aichan(const std::string &id, bool enabled,
                                      double range, coupling_t coupling,
                                      double range_offset) {
...
void digitizer_block_impl::set_aichan_range(const std::string &id, double range,
                                            double range_offset) {
  auto idx = convert_to_aichan_idx(id);
  d_channel_settings[idx].range = range;
  d_channel_settings[idx].offset = range_offset;
...
void digitizer_block_impl::set_aichan_trigger(const std::string &id,
                                              ...
                                              double threshold) {
  // Some scopes have an dedicated AUX Trigger-Input. Skip id verification for
  // them
  if (id != "AUX")
...
void digitizer_block_impl::set_diport(const std::string &id, bool enabled,
...
void digitizer_block_impl::set_di_trigger(uint32_t pin,
                                          trigger_direction_t direction) {
...
void digitizer_block_impl::configure() {
  ...
  d_app_buffer.initialize(get_enabled_aichan_count(),
                          get_enabled_diport_count(), d_buffer_size,
                          d_nr_buffers);
}
...
std::vector<error_info_t> digitizer_block_impl::get_errors() {
  return d_errors.get();
}
...
bool digitizer_block_impl::stop() {
  ...
  if (d_armed) {
    // Interrupt waiting function (workaround). From the scheduler point of view
    // this is not needed because it makes sure that the worker thread gets
    // interrupted before the stop method is called. But we have this in place
    // in order to allow for manual intervention.
...
int digitizer_block_impl::work_rapid_block(int noutput_items,
  ...
  if (d_bstate.state == rapid_block_state_t::READING_PART1) {
    ...
    // Instruct the driver to prefetch samples. Drivers might choose to ignore
    // this call
    ...
    ec = driver_get_rapid_block_data(d_bstate.offset, noutput_items,
                                     d_bstate.waveform_idx, output_items,
                                     d_status);
    if (ec) {
      add_error_code(ec);
      return -1;
...
void digitizer_block_impl::poll_work_function() {
  ...
  while (true) {
    ...
    if (state == poller_state_t::RUNNING) {
      ...
      // Watchdog is "turned on" only some time after the acquisition start for
      // two reasons:
      ...
      float estimated_samp_rate = 0.0;
      {
        ...
        // order to make the base class relatively generic we use mutex
        // (streaming callback is called from this
        //  thread).
      ...
      if (estimated_samp_rate <
        ...
        GR_LOG_ERROR(d_logger, "Watchdog: estimated sample rate " +
                                   std::to_string(estimated_samp_rate) +
                                   "Hz, expected: " +
                                   std::to_string(get_samp_rate()) + "Hz");
        d_app_buffer.notify_data_ready(digitizer_block_errc::Watchdog);
      }
      boost::chrono::duration<float> poll_duration =
          boost::chrono::high_resolution_clock::now() - poll_start;
...
void digitizer_block_impl::transit_poll_thread_to_idle() {
  ...
  d_poller_cv.wait(lock,
                   [this] { return d_poller_state == poller_state_t::IDLE; });
}
...
int digitizer_block_impl::work_stream(int noutput_items,
                                      gr_vector_void_star &output_items) {
  ...
  auto lost_count =
      d_app_buffer.get_data_chunk(ai_buffers, ai_error_buffers, port_buffers,
                                  channel_status, timestamp_now_ns_utc);
  ...
  if (lost_count) {
    GR_LOG_ERROR(
        ...
        std::to_string(lost_count) +
            " digitizer data buffers lost. Usually the cause of this error is, "
            "that the work method of the Digitizer block is called with low "
            "frequency because of a 'traffic jam' in the flowgraph. (One of "
            "the next blocks cannot process incoming data in time)");
  }
  ...
  for (auto trigger_offset : trigger_offsets) {
    //       std::cout << "trigger_offset       : " <<
    //       trigger_offset<<std::endl; std::cout << "noutput_items        : "
    //       << noutput_items <<std::endl; std::cout << "d_time_per_sample_ns :
    //       " << time_per_sample_with_downsampling_ns <<std::endl; std::cout <<
    //       "local_timstamp        : " << local_timstamp << std::endl;
    //       std::cout << "another now           :" << now << std::endl;
    //       std::cout << "timestamp_now_ns_utc  : " <<
    //       timestamp_now_ns_utc<<std::endl; std::cout << "diff[ns] : " <<
    //       uint64_t((noutput_items - trigger_offset ) *
    //       time_per_sample_with_downsampling_ns )<<std::endl; std::cout <<
    //       "stamp added           : " << uint64_t(timestamp_now_ns_utc - ((
    //       noutput_items - trigger_offset ) *
    //       time_per_sample_with_downsampling_ns )) <<std::endl; std::cout <<
    //       "tag offset: " << nitems_written(0) + trigger_offset <<std::endl;
    auto trigger_tag = make_trigger_tag(
        d_downsampling_factor,
...
int digitizer_block_impl::work(int noutput_items,
  ...
  if ((retval > 0) && !d_timebase_published) {
    auto timebase_tag =
        make_timebase_info_tag(get_timebase_with_downsampling());
