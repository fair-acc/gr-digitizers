 ...
 * You may use, distribute and modify this code under the terms of the GPL v.3
 * license.
 */
...
/*!
 * This method is needed because PICO_STATUS is not a distinct type (e.g. an
...
void validate_desired_actual_frequency_ps6000(double desired_freq,
                                              double actual_freq) {
  // In order to prevent exceptions/exit due to rounding errors, we dont
  // directly compare actual_freq to desired_freq, but instead allow a
  // difference up to 0.001%
...
/*!
 * Note this function has to be called after the call to the ps6000SetChannel
 * function, that is just before the arm!!!
 */
uint32_t
picoscope_6000_impl::convert_frequency_to_ps6000_timebase(double desired_freq,
                                                          double &actual_freq) {
  ...
  // limitation we use the ps6000GetTimebase2 function to find the closest
  // possible timebase. The below timebase estimate is therefore used as a
  // fallback only.
  auto time_interval_ns = 1000000000.0 / desired_freq;
  uint32_t timebase_estimate =
      (static_cast<uint32_t>(time_interval_ns / 6.4)) + 4;

  // In order to cover for all possible 30000 series devices, we use
  // ps6000GetTimebase2 function to get step size in ns between timebase 3
  // and 4. Based on that the actual timebase is calculated.
  uint32_t dummy;
  std::array<float, 2> time_interval_ns_56;

  for (auto i = 0; i < 2; i++) {
    auto status = ps6000GetTimebase2(d_handle, 5 + i, 1024,
                                     &time_interval_ns_56[i], 0, &dummy, 0);
    if (status != PICO_OK) {
      GR_LOG_NOTICE(d_logger, "timebase cannot be obtained: " +
                                  ps6000_get_error_message(status));
      GR_LOG_NOTICE(d_logger, "    estimated timebase will be used...");

      float time_interval_ns;
      status = ps6000GetTimebase2(d_handle, timebase_estimate, 1024,
                                  &time_interval_ns, 0, &dummy, 0);
      if (status != PICO_OK) {
        std::ostringstream message;
        message << "Exception in " << __FILE__ << ":" << __LINE__
  ...
  // Calculate steps between timebase 3 and 4 and correct start_timebase
  // estimate based on that
  auto step = time_interval_ns_56[1] - time_interval_ns_56[0];
  timebase_estimate = static_cast<uint32_t>(
                          (time_interval_ns - time_interval_ns_56[0]) / step) +
                      5;

  // The below code iterates trought the neighbouring timebases in order to find
  // the best match. In principle we could check only timebases on the left and
  // right but since first three timebases are in most cases special we make
  // search space a bit bigger.
  ...
  for (auto i = 0; i < search_space; i++) {
    ...
    auto status = ps6000GetTimebase2(d_handle, start_timebase + i, 1024,
                                     &obtained_time_interval_ns, 0, &dummy, 0);
    if (status != PICO_OK) {
...
int16_t convert_voltage_to_ps6000_raw_logic_value(double voltage,
                                                  float channel_range) {
...
picoscope_6000::sptr picoscope_6000::make(std::string serial_number,
                                          bool auto_arm) {
...
picoscope_6000_impl::picoscope_6000_impl(std::string serial_number,
                                         bool auto_arm)
    : gr::sync_block("picoscope_6000", gr::io_signature::make(0, 0, 0),
                     gr::io_signature::make(8, 8, sizeof(float))),
      picoscope_impl(serial_number, PS6000_MAX_CHANNELS,
                     2, // it seems no 3000 series device supports 4 ports
                     auto_arm,
                     255,   // max raw value
                     0.03), // vertical precision
      d_handle(-1), d_overflow(0) {
...
std::string picoscope_6000_impl::get_unit_info_topic(PICO_INFO info) {
  char line[40];
  int16_t required_size;

  auto status = ps6000GetUnitInfo(d_handle, reinterpret_cast<int8_t *>(line),
                                  sizeof(line), &required_size, info);
  if (status == PICO_OK) {
...
std::string picoscope_6000_impl::get_driver_version() {
...
std::string picoscope_6000_impl::get_hardware_version() {
  if (!d_initialized)
    return "NA";
...
std::error_code picoscope_6000_impl::driver_initialize() {
  ...
  if (d_serial_number.empty()) {
    ...
    for (uint16_t i = 0; i < devCount; i++) {
      ...
      ps6000GetUnitInfo(temp_handles[i], serial, sizeof(serial), &requiredSize,
  ...
  ////        std::cout << "serial:" << serial << std::endl;
  //        status = ps6000OpenUnit(&(d_handle),
  //        (int8_t*)d_serial_number.c_str());
  //      }
  //      std::cout << "picoscope_6000_impl::driver_initialize 2" << std::endl;
  //      if (status != PICO_OK) {
  //        GR_LOG_ERROR(d_logger, "open unit failed: " +
  //        ps6000_get_error_message(status)); return
  //        make_pico_6000_error_code(status);
  //      }
...
std::error_code picoscope_6000_impl::driver_configure() {
  ...
  PICO_STATUS status =
      ps6000MemorySegments(d_handle, d_nr_captures, &max_samples);
  if (status != PICO_OK) {
  ...
  for (auto i = 0; i < d_ai_channels; i++) {
    ...
    status = ps6000SetChannel(d_handle, static_cast<PS6000_CHANNEL>(i), enabled,
                              coupling, range, offset, PS6000_BW_FULL);
    if (status != PICO_OK) {
  ...
  if (d_trigger_settings.is_enabled() &&
    ...
    if (d_trigger_settings.source == "A")
      ...
      channel_range = 1.; // AUX can only be used for triggering, has a fixed
                          // Range of +/-1V according to Programmers guideline
    else {
      std::ostringstream message;
      message << "Exception in " << __FILE__ << ":" << __LINE__
    ...
    status = ps6000SetSimpleTrigger(
        ...
        -1); // auto trigger
    if (status != PICO_OK) {
      GR_LOG_ERROR(d_logger, "ps6000SetSimpleTrigger: " +
                                 ps6000_get_error_message(status));
      return make_pico_6000_error_code(status);
    }
    std::ostringstream message;
    ...
    if (status != PICO_OK) {
      GR_LOG_ERROR(d_logger, "ps6000SetTriggerChannelConditionsV2: " +
                                 ps6000_get_error_message(status));
      return make_pico_6000_error_code(status);
    }
  }
...
void rapid_block_callback_redirector_6000(int16_t handle, PICO_STATUS status,
                                          void *vobj) {
  static_cast<picoscope_6000_impl *>(vobj)->rapid_block_callback(handle,
                                                                 status);
}

void picoscope_6000_impl::rapid_block_callback(int16_t handle,
                                               PICO_STATUS status) {
...
std::error_code picoscope_6000_impl::driver_arm() {
  if (d_acquisition_mode == acquisition_mode_t::RAPID_BLOCK) {
    ...
    auto status = ps6000RunBlock(
        ...
        0,              // segment index
        (ps6000BlockReady)rapid_block_callback_redirector_6000, this);
    if (status != PICO_OK) {
      GR_LOG_ERROR(d_logger,
                   "ps6000RunBlock: " + ps6000_get_error_message(status));
      return make_pico_6000_error_code(status);
    }
  } else {
    ...
    auto status =
        ps6000RunStreaming(d_handle,
                           ...
                           d_driver_buffer_size, false, d_downsampling_factor,
...
std::error_code picoscope_6000_impl::driver_close() {
  ...
  if (status != PICO_OK) {
    GR_LOG_ERROR(d_logger,
                 "ps6000CloseUnit: " + ps6000_get_error_message(status));
  }
...
std::error_code picoscope_6000_impl::set_buffers(size_t samples,
  ...
  for (auto aichan = 0; aichan < d_ai_channels; aichan++) {
    ...
    if (d_downsampling_mode ==
        downsampling_mode_t::DOWNSAMPLING_MODE_MIN_MAX_AGG) {
      d_buffers[aichan].reserve(samples);
      d_buffers_min[aichan].reserve(samples);

      status = ps6000SetDataBuffers(
          d_handle, static_cast<PS6000_CHANNEL>(aichan), &d_buffers[aichan][0],
          &d_buffers_min[aichan][0], samples,
          convert_to_ps6000_ratio_mode(d_downsampling_mode));
    } else {
      d_buffers[aichan].reserve(samples);

      status = ps6000SetDataBuffer(
          d_handle, static_cast<PS6000_CHANNEL>(aichan), &d_buffers[aichan][0],
          samples, convert_to_ps6000_ratio_mode(d_downsampling_mode));
    }

    if (status != PICO_OK) {
      GR_LOG_ERROR(d_logger, "ps6000SetDataBuffer (chan " +
                                 std::to_string(aichan) +
                                 "): " + ps6000_get_error_message(status));
      return make_pico_6000_error_code(status);
    }
  }
...
std::error_code
picoscope_6000_impl::driver_prefetch_block(size_t samples,
                                           size_t block_number) {
  ...
  auto status =
      ps6000GetValues(d_handle,
                      0, // offset
                      &nr_samples, d_downsampling_factor,
                      convert_to_ps6000_ratio_mode(d_downsampling_mode),
                      block_number, &d_overflow);
  if (status != PICO_OK) {
    GR_LOG_ERROR(d_logger,
                 "ps6000GetValues: " + ps6000_get_error_message(status));
  }

  return make_pico_6000_error_code(status);
}

std::error_code picoscope_6000_impl::driver_get_rapid_block_data(
    size_t offset, size_t length, size_t waveform, gr_vector_void_star &arrays,
    std::vector<uint32_t> &status) {
  int vec_index = 0;

  for (auto chan_idx = 0; chan_idx < d_ai_channels;
       chan_idx++, vec_index += 2) {
    ...
    if (d_downsampling_mode == downsampling_mode_t::DOWNSAMPLING_MODE_NONE ||
        d_downsampling_mode ==
            downsampling_mode_t::DOWNSAMPLING_MODE_DECIMATE) {
...
std::error_code picoscope_6000_impl::driver_poll() {
  auto status = ps6000GetStreamingLatestValues(
      d_handle, (ps6000StreamingReady)invoke_streaming_callback,
