...
int16_t convert_voltage_to_ps6000_raw_logic_value(double value) {
...
std::error_code picoscope_6000_impl::driver_initialize() {
  ...
  if (d_serial_number.empty()) {
    ...
    for (uint16_t i = 0; i < devCount; i++) {
      ...
      GR_LOG_INFO(d_logger,
                  "... found scope with serial: '" + convert.str() + "'");
    ...
    if (found)
      ...
      GR_LOG_ERROR(d_logger, "No matching serial found for scope '" +
                                 d_serial_number + "'");
  }
