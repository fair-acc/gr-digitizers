...
#include <pulsed_power_daq/digitizer_source.h>
...
namespace gr {
  namespace pulsed_power_daq {
   ...
   uint32_t
   digitizer_source::get_pre_trigger_samples_with_downsampling() const
   {
   ...
   uint32_t
   digitizer_source::get_post_trigger_samples_with_downsampling() const
   {
     if (d_downsampling_mode != downsampling_mode_t::DOWNSAMPLING_MODE_NONE)
   ...
   uint32_t
   digitizer_source::get_block_size() const
   {
     return d_post_samples + d_pre_samples;
   }

   uint32_t
   digitizer_source::get_block_size_with_downsampling() const
   {
       return get_pre_trigger_samples_with_downsampling() + get_post_trigger_samples_with_downsampling();
   }

   double
   digitizer_source::get_timebase_with_downsampling() const
   {
     if (d_downsampling_mode == downsampling_mode_t::DOWNSAMPLING_MODE_NONE) {
   ...
   void
   digitizer_source::add_error_code(std::error_code ec)
   {
     d_errors.push(ec);
   }

   std::vector<int>
   digitizer_source::find_analog_triggers(float const * const samples, int nsamples)
   {
   ...
   std::vector<int>
   digitizer_source::find_digital_triggers(uint8_t const * const samples, int nsamples, uint8_t mask)
   {
   ...
   acquisition_mode_t
   digitizer_source::get_acquisition_mode()
   {
     return d_acquisition_mode;
   }

   void
   digitizer_source::set_samples(int pre_samples, int post_samples)
   {
     if (post_samples < 1)
     {
   ...
   void
   digitizer_source::set_samp_rate(double rate)
   {
     if (rate <= 0.0)
     {
       std::ostringstream message;
   ...
   double
   digitizer_source::get_samp_rate()
   {
     return d_actual_samp_rate;
   }

   void
   digitizer_source::set_buffer_size(int buffer_size)
   {
     if (buffer_size < 0)
     {
       std::ostringstream message;
   ...
   void
   digitizer_source::set_nr_buffers(int nr_buffers)
   {
     if (nr_buffers < 1)
     {
       std::ostringstream message;
   ...
   void
   digitizer_source::set_driver_buffer_size(int driver_buffer_size)
   {
   ...
   void
   digitizer_source::set_auto_arm(bool auto_arm)
   {
     d_auto_arm = auto_arm;
   }

   void
   digitizer_source::set_trigger_once(bool once)
   {
     d_trigger_once = once;
   }
   ...
   void
   digitizer_source::set_streaming(double poll_rate)
   {
     if (poll_rate < 0.0)
     {
       std::ostringstream message;
   ...
   void
   digitizer_source::set_rapid_block(int nr_captures)
   {
     if (nr_captures < 1)
     {
       std::ostringstream message;
   ...
   void
   digitizer_source::set_downsampling(downsampling_mode_t mode, int downsample_factor)
   {
   ...
   int
   digitizer_source::convert_to_aichan_idx(const std::string &id) const
   {
     if (id.length() != 1)
     {
       std::ostringstream message;
   ...
   void
   digitizer_source::set_aichan(const std::string &id, bool enabled, double range, coupling_t coupling, double range_offset)
   {
   ...
   int
   digitizer_source::get_enabled_aichan_count() const
   {
     auto count = 0;
   ...
   void
   digitizer_source::set_aichan_range(const std::string &id, double range, double range_offset)
   {
     auto idx = convert_to_aichan_idx(id);
     d_channel_settings[idx].range = range;
     d_channel_settings[idx].offset = range_offset;
   ...
   void
   digitizer_source::set_aichan_trigger(const std::string &id, trigger_direction_t direction, double threshold)
   {
   ...
   int
   digitizer_source::convert_to_port_idx(const std::string &id) const
   {
     if (id.length() != 5)
     {
       std::ostringstream message;
   ...
   void
   digitizer_source::set_diport(const std::string &id, bool enabled, double thresh_voltage)
   {
   ...
   int
   digitizer_source::get_enabled_diport_count() const
   {
     auto count = 0;
   ...
   void
   digitizer_source::set_di_trigger(uint32_t pin, trigger_direction_t direction)
   {
   ...
   void
   digitizer_source::disable_triggers()
   {
     d_trigger_settings.source = TRIGGER_NONE_SOURCE;
   }

   void
   digitizer_source::initialize()
   {
     if (d_initialized) {
       return;
     }
   ...
   bool
   digitizer_source::is_armed()
   {
     return d_armed;
   }

   void
   digitizer_source::disarm()
   {
     if (!d_armed) {
       return;
     }
   ...
   digitizer_source::get_errors()
   {
     return d_errors.get();
   }

   std::string
   digitizer_source::getConfigureExceptionMessage()
   {
       return d_configure_exception_message;
   }

   bool
   digitizer_source::start()
   {
     try {
       initialize();
       configure();
       ...
       d_configure_exception_message = "Unknown Exception received in digitizer_source::start";
       return false;
     }
   ...
   void
   digitizer_source::notify_data_ready(std::error_code ec)
   {
     if(ec) {
       add_error_code(ec);
     }
   ...
   std::error_code
   digitizer_source::wait_data_ready()
   {
     boost::mutex::scoped_lock lock(d_mutex);
   ...
   int
   digitizer_source::work_rapid_block(int noutput_items, gr_vector_void_star &output_items)
   {
   ...
   void
   digitizer_source::stop_poll_thread()
   {
     if (!d_poller.joinable()) {
       return;
     }
   ...
   int
   digitizer_source::work_stream(int noutput_items, gr_vector_void_star &output_items)
   {
